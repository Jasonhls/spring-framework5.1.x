...
public class HandlerMethodReturnValueHandlerComposite implements HandlerMethodReturnValueHandler {
	...
	private HandlerMethodReturnValueHandler selectHandler(@Nullable Object value, MethodParameter returnType) {
		...
		/**
		 *this.returnValueHandlers已经在RequestMappingHandlerAdapter的afterPropertiesSet方法中被初始化了
		 */
		for (HandlerMethodReturnValueHandler handler : this.returnValueHandlers) {
			...
			/**
			 * 判断返回处理器是否支持返回类型，如果handler是RequestResponseBodyMethodProcessor，
			 * 那么判断逻辑是returnType对应的类上是否含有@ResponseBody注解或returnType对应的类是否带有@ResponseBody注解的方法
			 */
